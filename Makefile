...
test-e2e:
    ...
    # Ejecuta Cypress con reporter JUnit y guarda el resultado en /results/e2e_result.xml
    docker start -a e2e-tests || true
    docker exec e2e-tests npx cypress run --reporter junit --reporter-options "mochaFile=/results/e2e_result.xml,toConsole=true"
